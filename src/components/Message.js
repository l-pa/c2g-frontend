...
function Message(props) {
  ...
  useEffect(() => {
    if (props.userId !== 'System' && props.message.match(re)) {
      setMatched(true)
      const a = [...props.message.matchAll(re)]
      let matches = []
      a.forEach(arr => {
        matches.push(arr[1])
      })
      ...
      for (let i = 0; i < matches.length; i++) {
        const element = matches[i].toLowerCase();

        const obj = emotes.find(o => o.code === element);

        if (obj) {
          setGifUrl(obj.url, true)
        } else {
          setMatched(false)
        }
      ...
      const uniqueEmotes = [...new Set(matches)]

      for (let i = 0; i < uniqueEmotes.length; i++) {
        const element = uniqueEmotes[i]
        const obj = emotes.find(o => o.code === element);
        if (obj) {
          if ('audio' in obj) new Audio(obj.audio).play()
        }
        /*
         switch (element) {
           case ':medic:':
